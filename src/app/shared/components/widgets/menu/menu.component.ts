...
export class MenuComponent {
  ...
  // Returns a local SVG logo path for a nav item when no custom item_image is set.
  // Maps common category names to fashion-themed SVG assets.
  iconFor(title: string | undefined | null): string {
    const base = 'assets/svg/fashion/';
    if (!title) return base + 't-shirt.svg';
    const t = title.toString().toLowerCase().trim();
    if (/active\s*wear|sport|gym|athleisure|swim/.test(t)) return base + 'swimwear.svg';
    if (/women|female|ladies|girl|dress|gown|ethnic|saree|kurta/.test(t)) return base + 'gown.svg';
    if (/men|male|boy|gents|blazer|suit/.test(t))         return base + 'blzer.svg';
    if (/kid|child|baby|infant/.test(t))                  return base + 't-shirt.svg';
    if (/jacket|coat|winter|hoodie|sweat/.test(t))        return base + 'jacket.svg';
    if (/denim|jean|pant|trouser|bottom|cord/.test(t))    return base + 'jeans.svg';
    if (/shirt|t-shirt|top|tee/.test(t))                  return base + 't-shirt.svg';
    if (/sleep|night|loungewear|pajama/.test(t))          return base + 'sleepwear.svg';
    if (/shape|lingerie|innerwear/.test(t))               return base + 'shapewear.svg';
    return base + 't-shirt.svg';
  }
