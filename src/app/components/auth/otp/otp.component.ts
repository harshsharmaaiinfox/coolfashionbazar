...
import { VerifyEmailOtp, VerifyNumberOTP, VerifyRegistrationOtp, VerifyLoginOtp } from '../../../shared/action/auth.action';
...
export class OtpComponent {
  ...
  // OTP length: login and registration use 6 digits; forgot-password and phone login use 5 digits.
  get otpLength(): number {
    return (this.otpType === 'register' || this.otpType === 'login') ? 6 : 5;
  }
  ...
  submit() {
    ...
    if(this.otpType === 'register') {
      action = new VerifyRegistrationOtp({ email: this.email, token: otp });
    }

    if(this.otpType === 'login') {
      action = new VerifyLoginOtp({ email: this.email, token: otp });
    }
