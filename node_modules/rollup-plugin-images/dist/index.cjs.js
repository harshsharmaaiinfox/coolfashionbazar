...
function img(opt) {
  ...
  return {
    ...
    load: function load(id) {
      ...
      if (fs.statSync(id).size <= (opt.limit || 8192)) {
        // use base64
        return ("export default { uri : \"data:" + (mimeMap[ext]) + ";base64," + (fs.readFileSync(id, 'base64')) + "\" }");
      } else {
        ...
        return ("const img = require('./" + (opt._slash ? './' : '') + (outputFile.slice(baseIndex)) + "'); export default img;");
      }
    },
  };
}
